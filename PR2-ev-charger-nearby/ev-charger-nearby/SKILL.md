...
- `zscode` (선택, 권장) — 시군구코드 5자리 (예: `11680` 강남구). 주면 한 번의 호출로 끝나고 결과가 더 가깝다. **경계 근처면 콤마로 인접 구를 함께** 넣을 수 있다 (예: `11680,11650` 강남+서초, 최대 5개) → 경계 너머 충전소 누락 방지.
...
> `zscode` 를 주면 해당 시·군·구만 조회하므로 빠르고 정확하다. `zcode` 만 주면 시·도 전체를 페이지 단위로 긁기 때문에 무겁다(예: 서울 전체 충전기 약 7.4만 건 → 여러 페이지). 가능하면 사용자 위치를 시·군·구까지 좁혀 `zscode` 를 함께 보낸다. 응답에 `truncated: true` 와 사람이 읽을 `notice` 메시지가 오면, 시·도가 너무 커서 일부만 조회됐다는 뜻이므로 그 `notice` 를 사용자에게 그대로 전하고 `zscode`/`regionHint` 로 좁혀 다시 조회한다.
