# feat(seoul-bike-share): 서울 따릉이 실시간 대여소 조회 스킬 + 프록시 라우트 2종
...
- **두 라우트 분리**: nearest 는 proxy 측에서 전체 페이지 호출·거리 계산·top N 슬라이스까지 마쳐 클라이언트 응답을 작게 유지. stations 는 디버깅/dump 용 pass-through 로 별도 분리.
...
| `packages/k-skill-proxy/src/server.js` | 수정 — import 블록, `handleSeoulBikeRoute`, 라우트 2 개 등록, exports 추가 |
...
| `packages/k-skill-proxy/README.md` | 수정 — 라우트 목록 2 줄 추가 |
| `.changeset/seoul-bike-share.md` | 신규 — `k-skill-proxy` minor |

## 검증

- [ ] `node --test packages/k-skill-proxy/test/server.test.js` — seoul-bike 5 개 신규 케이스 포함 통과
- [ ] `./scripts/validate-skills.sh`
- [ ] `npm run lint`
- [ ] (리뷰어) `curl "$BASE/v1/seoul-bike/nearest?lat=37.4979&lng=127.0276&limit=5"` — 프록시가 `SEOUL_OPEN_API_KEY` 보유하고 `OA-13252` 활용신청 승인된 환경에서 실제 따릉이 대여소 5 개 + `distanceMeters` + `availableRacks` 가 정상 응답되는지 확인
